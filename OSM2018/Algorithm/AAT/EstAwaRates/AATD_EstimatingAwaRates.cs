...
{
    class AATD_EstimatingAwaRates : I_EstimatingAwaRates
    {
        ...
        void UpdateAveAwaRates(int current_round, I_Agent agent, I_CandidateSet canset)
        {
            ...
            var receive_rounds = agent.ReceiveRoundList.Count;

            foreach (var can in canset.SingleCandidateList)
            {
                var pre_counts = (receive_rounds - 1) * can.AwaRate;

                if (this.IsEvsOpinionFormed(agent, select_can, can, canset))
                {
                    can.AwaRate = (pre_counts + 1) / receive_rounds;
                    can.AwaCounts = (int)pre_counts + 1;
                    can.AwaCurrentRounds = receive_rounds;
                }
                else
                {
                    can.AwaRate = (pre_counts + 0) / receive_rounds;
